...
func getMinuteAsleep(gs []*guardschedule) (maxID, maxValI int) {
	maxVal := 0
	for _, valslice := range gs {
		for i, val := range valslice.minutessleeping {
			if maxVal < val {
				maxVal = val
				maxValI = i
				maxID = valslice.guardID
			}
		}
	}
	return
